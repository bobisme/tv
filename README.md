...
During development, use `cargo run --release -- path/to/document.md`. For example:

```sh
tv ~/src/ward/ward-slm/one_model/campaign-2026-09-24-clm/RESULTS.md
```

Markdown is parsed in memory and typeset with a NeurIPS-inspired paper layout based on the report's companion `RESULTS.typ`: US Letter pages, ruled title, compact serif text, an abstract, tables, and page numbers. The first H1 is used as the title; otherwise the filename is used. A long opening paragraph becomes the abstract. Tables, links, lists, code blocks, emphasis, and local images are supported. The Markdown file and local images are watched for changes. Math is shown verbatim because Markdown and Typst use different math syntax.
